...
func fixtureCreateLoadBalancerPayload(mods ...func(m *alb.CreateLoadBalancerPayload)) *alb.CreateLoadBalancerPayload {
	resp := &alb.CreateLoadBalancerPayload{}
	for _, mod := range mods {
		mod(resp)
	}
	return resp
...
func fixtureUpdateLoadBalancerPayload(mods ...func(m *alb.UpdateLoadBalancerPayload)) *alb.UpdateLoadBalancerPayload {
	resp := &alb.UpdateLoadBalancerPayload{}
	for _, mod := range mods {
		mod(resp)
	}
	return resp
...
func TestToCreatePayload(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected:    fixtureCreateLoadBalancerPayload(),
			isValid:     true,
		},
		{
			"simple_values_ok",
									...
									map[string]attr.Value{
										"hosts": types.SetValueMust(
											...
											[]attr.Value{types.ObjectValueMust(
												...
												map[string]attr.Value{
													"host":  types.StringValue("*"),
													"rules": types.SetNull(types.ObjectType{AttrTypes: ruleTypes}),
												}),
											},
										),
									},
